...
def _setup_prototypes(lib):
    ...
    lib.libusb_get_configuration.argtypes = [
        _libusb_device_handle, POINTER(c_int)]
...
class _IsoTransferHandler(_objfinalizer.AutoFinalizedObject):
    def __init__(self, dev_handle, ep, buff, timeout):
        ...
        packet_length = _lib.libusb_get_max_iso_packet_size(
            dev_handle.devid, ep)
...
class _LibUSB(usb.backend.IBackend):
    ...
    def get_device_descriptor(self, dev):
        ...
        _check(self.lib.libusb_get_device_descriptor(
            dev.devid, byref(dev_desc)))
    ...
    def set_configuration(self, dev_handle, config_value):
        _check(self.lib.libusb_set_configuration(
            dev_handle.handle, config_value))

    @methodtrace(_logger)
    def get_configuration(self, dev_handle):
        config = c_int()
        _check(self.lib.libusb_get_configuration(
            dev_handle.handle, byref(config)))
        return config.value
