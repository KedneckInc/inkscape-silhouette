...
class MatFree:
    def __init__(self, preset="default", scale=1.0, pen=None):
        ...
        # used in both, simple_barrier() and pyramids_barrier()
        self.monotone_back_travel = 3.0
        ...
        if self.min_segmentlen < 0.001:
            self.min_segmentlen = 0.001
    ...
    def preset(self, pre_name):
        if not pre_name in presets:
            raise ValueError(pre_name+': no such preset. Try "' +
                             '", "'.join(presets.keys())+'"')
    ...
    def load(self, cut):
        ...
        for path in cut:
            new_path = []
            for point in path:
                idx = self.pt2idx(self.input_scale *
                                  point[0], self.input_scale * point[1])
            ...
            for pt in path:
                if len(new_path):
                    ...
                    if dist_a_pt_sq > maxlen_sq:
                        ...
                        if s.verbose > 1:
                            print >>sys.stderr, "pt%d -- pt%d: need nsub=%d, seg_len=%g" % (
                                A, pt, nsub, seg_len)
                            print >>sys.stderr, "dxdy", dx, dy, "to", (
                                s.points[pt].x, s.points[pt].y), "from", (s.points[A].x, s.points[A].y)
                        for subdiv in range(nsub):
                            ...
                            if s.verbose > 1:
                                print >>sys.stderr, "   sub", (
                                    s.points[sub_pt].x, s.points[sub_pt].y)
                new_path.append(pt)
    ...
    def append_or_extend_hard(s, seg):
        ...
        if (len(s.output) > 0 and len(s.output[-1]) >= 2 and
            ...
            if (s.output[-1][0].id == seg[0].id or
                ...
                if s.verbose:
                    print >>sys.stderr, "late flip ", len(
                        s.output), len(s.output[-1])
            #
        #
    ...
    def shortcut_segment(self, A, B, C):
        ...
        if None in (a_seg_idx, b_seg_idx, c_a_seg_idx, c_b_seg_idx):
            raise ValueError(
                "shortcut_segment cannot find [AC] and [CB] seg.", a_seg_idx, b_seg_idx, c_a_seg_idx, c_b_seg_idx)
    ...
    def process_pyramids_barrier(s, y_slice, max_y, left2right=True):
        ...
        """
        Xf_bar = Barrier(
            y_slice, key=lambda a:  a[0]+a[1] if a else 0)  # forward:   / moving ->
        # backwards: \ moving <-
        Xb_bar = Barrier(y_slice, key=lambda a: -a[0]+a[1] if a else 0)

        if not left2right:                                        # forward:   \ moving <-
            # backwards: / moving ->
            Xf_bar, Xb_bar = Xb_bar, Xf_bar
        ...
        while True:
            ...
            for Bi in A.seg:
                if Bi >= 0:                              # segment already done
                    ...
                    if A.y+s.min_segmentlen >= max_y and pt.y > A.y:
                        # Do not look downward when close to max_y.
                        continue
                        # This avoids a busyloop after hitting Y_bar:
                        # ... subdiv, advance, retry, hit, subdiv, ...
                    if left2right:
                        if pt.x >= A.x:
                            # find the leftmost of all [AB]
                            if B is None or not ccw(A, B, pt):
                                B = pt
                    else:  # not left2right
                        if pt.x <= A.x:
                            # find the rightmost of all [AB]
                            if B is None or ccw(A, B, pt):
                                B = pt
            ...
            if (subdividable_ab and (
                ...
                # could also use lookup() here. It does not matter.
                ...
                for n in range(Xb_a_idx, Xb_b_idx+1):   # sweep from A to B
                    ...
                    if pt.id != A.id and pt.id != B.id and ccw(A, B, pt) == True:
                        # found a D that is clearly left of AB.
                        D = pt
                        break
                    else:
                        print "backsweep ignoring pt", pt, Xb_bar.key(pt)
                #
                # compute intersection of Xb_bar with [AB]
                if D is not None:
                    E = intersect_lines(
                        D, XY_a((D.x+1, D.y+1)), A, B, limit2=True)
                    if E is None:
                        raise ValueError(
                            "finding a shadowed D failed:", A, B, D)
                    E = XY_a(E)
                    s.subdivide_segment(A, B, E)
                    Xf_bar.insert(E)
                    Xb_bar.insert(E)
                    if C is not None:
                        # replace segments [EB], [BC] with [EC]
                        s.shortcut_segment(E, C, B)
                        B, C = C, B
                    B, E = E, B

            # tentatively advance Xf_bar from A to B
            # unused, we never move back to A.
            Xf_a_idx = Xf_bar.pos()
            Xf_b_idx = Xf_bar.lookup(lambda b: b.id == B.id if b else False)
            if Xf_b_idx is None:                      # Should never happen!
                # Okayish fallback, but find() may return
                print "Xf_bar.lookup(B)=None. B=", B
                # a different point with the same key().
            ...
            for n in range(Xf_a_idx, Xf_b_idx+1):     # sweep from A to B (inclusive)
                ...
                if subdividable_ab and pt.id != A.id and pt.id != B.id and ccw(A, B, pt) == (not left2right):
                    # found an F that is clearly right of AB.
                    F = pt
                    Xf_f_idx = n
                    break
                else:
            ...
            if F is not None:
                ...
                if G is None:
                    raise ValueError(
                        "finding a shadowed G failed:", A, B, F, _F_back)
                G = XY_a(G)
                ...
                # advance to F, further up on the same barrier as G
                Xf_bar.pos(Xf_f_idx)
                A = Xf_bar.point()
            #
            else:
        ...
        print >>sys.stderr, "barrier moved all the way", Xf_bar.points, max_y, A.att(
        ) if A else None, A.seg if A else None
    ...
    def process_simple_barrier(s, y_slice, max_y, last_x=0.0):
        ...
        """
        if s.verbose:
            print >>sys.stderr, "process_simple_barrier limit=%g, points=%d, %s" % (
                max_y, len(y_slice), last_x)
        ...
        for pt in y_slice:
            ...
            for iC in pt.seg:
                ...
                if C is not None and C.y <= max_y:
                    ...
                    if min_x is None or min_x > C.x:
                        min_x = C.x
                    if min_x is None or min_x > pt.x:
                        min_x = pt.x
                    if max_x is None or max_x < C.x:
                        max_x = C.x
    ...
    def decide_left2right(s, min_x, max_x, last_x=0.0):
        ...
        """
        if min_x >= last_x:
            return True     # easy: all is to the right
        if max_x <= last_x:
            return False    # easy: all is to the left.
        if abs(last_x - min_x) < abs(max_x - last_x):
            # the left edge (aka min_x) is nearer
            return True
        else:
            # the right edge (aka max_x) is nearer
            return False
    ...
    def pyramids_barrier(s):
        ...
        while True:
            ...
            while Y_bar.point() is None:                        # skip forward dropped points
                ...
                # next() returns an idx, except when hitting the end.
                if Y_bar.next() is None:
                    break
            min_y = Y_bar.point().y
            ...
            s.process_pyramids_barrier(
                Y_bar.pslice(), barrier_y, left2right=dir_toggle)
    ...
    def simple_barrier(s):
        ...
        while True:
            ...
            if barrier_idx > old_idx:
                last_x = s.process_simple_barrier(
                    sy[0:barrier_idx], barrier_y, last_x=last_x)
            if barrier_idx >= len(sy):
                break
            barrier_y += s.barrier_increment
        #
    ...
    def apply_overshoot(s, paths, start_travel, end_travel):
        ...
        def extend_b(A, B, travel):
            ...
            if d < 0.000001:
                return B         # cannot extrapolate if A == B
            ratio = travel/d
            dx = B.x-A.x
            dy = B.y-A.y
    ...
    def apply(self, cut):
        ...
        if self.tool_pen == False and self.overshoot > 0.0:
            self.output = self.apply_overshoot(
                self.output, self.overshoot, self.overshoot)
