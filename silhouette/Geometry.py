...
def sharp_turn(A, B, C, fwd_ratio):
    ...
    """
    if fwd_ratio == 0.0:
        return sharp_turn_90(A, B, C)      # short cut.
...
def intersect_lines(A, B, C, D, limit1=False, limit2=False):
    ...
    def _in_segment(A, B, x, y):
        ...
        if (abs(A.x-B.x) > _eps):            # AB is not vertial, test x-coordinate
            if A.x <= x+_eps and x-_eps <= B.x:
                return True
            if A.x >= x-_eps and x+_eps >= B.x:
                return True
        else:                               # test y-coordinate
            if A.y <= y+_eps and y-_eps <= B.y:
                return True
            if A.y >= y-_eps and y+_eps >= B.y:
                return True
    ...
    if det < _eps and det > -_eps:
        ...
        if colinear(A, B, C) and colinear(A, B, D):
            if _in_segment(A, B, C.x, C.y):
                return C     # A--C--B--D or A--C--D--B
            if _in_segment(A, B, D.x, D.y):
                return D     # A--D--B--C
            if _in_segment(C, D, A.x, A.y):
                return A     # C--A--B--D
    ...
    if limit1 and not _in_segment(A, B, x, y):
        return None
    if limit2 and not _in_segment(C, D, x, y):
        return None
    return (x, y)
...
class Barrier:
    ...
    def insert(self, point):
        ...
        # if none is before, let insert become append.
