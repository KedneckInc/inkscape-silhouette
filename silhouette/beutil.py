...
def to_BE(x, y):
    if abs(x) < 112 and abs(y) < 112:
        ...
        index -= d2 * 224
        d1 = index
        be3 = "%02X%02X%02X%02X%02X" % (
            d1 + 0x20, d2 + 0x20, d3 + 0x20, d4 + 0x20, d5 + 0x20)
        # print("BE3:", be3)
        return ("BE3", be3)
    else:
...
def from_BE(be_stream):
    ...
    if len(be_stream) == 4:
        ...
        # end if
        index = (d5 - 0x20) * (224 * 224 * 224 * 224) + (d4 - 0x20) * (224 *
                                                                       224 * 224) + (d3 - 0x20) * (224 * 224) + (d2 - 0x20) * 224 + (d1 - 0x20)
...
def test_BE(x, y, be_stream, be_enc):
    ...
    print("to_BE: (%d, %d) -> '%s' %s= '%s' : %s" % (x, y, stream,
                                                     '=' if passed else '!', be_stream, "PASSED" if passed else "FAILED"))
    if not passed:
        sys.exit(-1)
    # end if
    ...
    print("from_BE: '%s -> (%d, %d) %s= (%d, %d) : %s" % (be_stream,
                                                          xy[0], xy[1], '=' if passed else '!', x, y, "PASSED" if passed else "FAILED"))
    if not passed:
        sys.exit(-1)
    # end if
