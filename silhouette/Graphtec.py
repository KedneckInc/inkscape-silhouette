...
sys.path.append(os.path.dirname(os.path.abspath(__file__)) +
                '/pyusb-1.0.2')      # have a pyusb fallback
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print("The python usb module could not be found. Try",
                      file=sys.stderr)
                print("\t sudo zypper in python-usb \t\t# if you run SUSE",
                      file=sys.stderr)
                print(
                    "\t sudo apt-get install python-usb   \t\t# if you run Ubuntu", file=sys.stderr)
                print("\n\n\n", file=sys.stderr)
                raise e2
...
try:
    try:
        ...
        if sys_platform.startswith('win'):
            usb_vi = 1
            # windows does not seem to detect the usb.version , gives attribute error. Other tests of pyusb work, pyusb is installed.
            pass
        usb_vi_str = 'unknown'

    if usb_vi < 1:
        print("Your python usb module appears to be "+usb_vi_str +
              " -- We need version 1.x", file=sys.stderr)
    ...
    # on OS X usb.version_info[0] will always fail as libusb1 is being used
    pass
...
DEVICE = [
    ...
    {'vendor_id': VENDOR_ID_GRAPHTEC, 'product_id': PRODUCT_ID_CC300_20,
        'name': 'Craft Robo CC300-20'},
    {'vendor_id': VENDOR_ID_GRAPHTEC,
        'product_id': PRODUCT_ID_SILHOUETTE_SD_1, 'name': 'Silhouette SD 1'},
    {'vendor_id': VENDOR_ID_GRAPHTEC,
        'product_id': PRODUCT_ID_SILHOUETTE_SD_2, 'name': 'Silhouette SD 2'},
]
...
def _bbox_extend(bb, x, y):
    ...
    if not 'llx' in bb or x < bb['llx']:
        bb['llx'] = x
    if not 'urx' in bb or x > bb['urx']:
        bb['urx'] = x
    if not 'lly' in bb or y > bb['lly']:
        bb['lly'] = y
    if not 'ury' in bb or y < bb['ury']:
        bb['ury'] = y
    ...
    def __init__(self, log=sys.stderr, no_device=False, progress_cb=None):
        ...
        if no_device is True:
            ...
            for hardware in DEVICE:
                if sys_platform.startswith('win'):
                    print(
                        "device lookup under windows not tested. Help adding code!", file=self.log)
                    dev = usb.core.find(
                        idVendor=hardware['vendor_id'], idProduct=hardware['product_id'])

                elif sys_platform.startswith('darwin'):
                    dev = usb1ctx.openByVendorIDAndProductID(
            ...
            if dev is None:
                if sys_platform.startswith('win'):
                    print(
                        "device fallback under windows not tested. Help adding code!", file=self.log)
                    ...
                    print(
                        "device fallback under macosx not implemented. Help adding code!", file=self.log)
            ...
            print("%s found on usb bus=%d addr=%d" %
                  (self.hardware['name'], dev_bus, dev_addr), file=self.log)

            if sys_platform.startswith('win'):
                print(
                    "device init under windows not implemented. Help adding code!", file=self.log)
                ...
                try:
                    if dev.is_kernel_driver_active(0):
                        print(
                            "is_kernel_driver_active(0) returned nonzero", file=self.log)
                        if dev.detach_kernel_driver(0):
                            print(
                                "detach_kernel_driver(0) returned nonzero", file=self.log)
...
Alternatively, you can add yourself to group 'lp' and logout/login.""" % (self.hardware['vendor_id'], self.hardware['product_id'])
        ...
        self.dev = dev
        # probably never needed, but harmful on some versions of usb.core
    ...
    def write(s, string, timeout=10000):
        ...
        try:
            ...
            if resp:
                print("response before write('%s'): '%s'" %
                      (string, resp), file=s.log)
        except:
            pass
        endpoint = 0x01
        chunksz = 4096
        ...
        while o < len(string):
            ...
            try:
                if s.need_interface:
                    try:
                        r = s.dev.write(endpoint, chunk,
                                        interface=0, timeout=timeout)
                    except AttributeError:
                        r = s.dev.bulkWrite(
                            endpoint, chunk, interface=0, timeout=timeout)
                else:
                    try:
                ...
                raise TypeError("Write Exception: %s, %s dev=%s" %
                                (type(te), te, type(s.dev)))
            except AttributeError as ae:
                # write() got an unexpected keyword argument 'interface'
                raise TypeError("Write Exception: %s, %s dev=%s" %
                                (type(ae), ae, type(s.dev)))
            ...
            if r == 0 and retry < 5:
                ...
                raise ValueError('write %d bytes failed: r=%d' %
                                 (len(chunk), r))
            else:
                retry = 0
            o += r
        ...
        if o != len(string):
            raise ValueError('write all %d bytes failed: o=%d' %
                             (len(string), o))
    ...
    def read(s, size=64, timeout=5000):
        ...
        if s.need_interface:
            try:
                ...
                data = s.dev.bulkRead(
                    endpoint, size, timeout=timeout, interface=0)
        else:
            try:
    ...
    def status(s):
        ...
        if resp[-1] != '\x03':
            raise ValueError(
                'status response not terminated with 0x03: %s' % (resp[-1]))
        if resp[:-1] == '0':
            return "ready"
        if resp[:-1] == '1':
            return "moving"
    ...
    def wait_for_ready(s, timeout=30, verbose=True):
        ...
        for i in range(1, int(timeout*.5)):
            if (state == 'ready'):
                break
            if verbose:
                print(" %d/%d: status=%s\r" %
                      (i, int(timeout*.5), state), end='', file=sys.stderr)
            if verbose == False:
                if state == 'unloaded':
                    print(" %d/%d: please load media ...\r" %
                          (i, int(timeout*.5)), end='', file=sys.stderr)
                elif i > 5:
                    print(" %d/%d: status=%s\r" %
                          (i, int(timeout*.5), state), end='', file=sys.stderr)
            time.sleep(2.0)
    ...
    def get_version(s):
        ...
        try:
            # Large timeout because the plotter moves.
    ...
    def setup(s, media=132, speed=None, pressure=None, toolholder=None, pen=None, cuttingmat=None, sharpencorners=False, sharpencorners_start=0.1, sharpencorners_end=0.1, autoblade=False, depth=None, sw_clipping=True, trackenhancing=False, bladediameter=0.9, landscape=False, leftaligned=None, mediawidth=210.0, mediaheight=297.0):
        ...
        if s.product_id() == PRODUCT_ID_SILHOUETTE_CAMEO3:
            ...
            if cuttingmat == 'cameo_12x12':
                ...
                s.write("\\%d,%d\x03Z%d,%d\x03" %
                        (0, 0, height * 20.0, width * 20.0))

        if media is not None:
            if media < 100 or media > 300:
                media = 300
            if s.product_id() == PRODUCT_ID_SILHOUETTE_CAMEO3:
            ...
            for i in MEDIA:
                if i[0] == media:
                    print("Media=%d, cap='%s', name='%s'" %
                          (media, i[4], i[5]), file=s.log)
                    if pressure is None:
                        pressure = i[1]
                    if speed is None:
                        speed = i[2]
        ...
        if s.product_id() == PRODUCT_ID_SILHOUETTE_CAMEO3:
            ...
            if pen:
                ...
                sharpencorners_start = int(
                    (sharpencorners_start + 0.05) * 10.0)
                sharpencorners_end = int((sharpencorners_end + 0.05) * 10.0)
                s.write("FF%d,0,%d\x03FF%d,%d,%d\x03" % (sharpencorners_start,
                                                         toolholder, sharpencorners_start, sharpencorners_end, toolholder))
        ...
        if s.product_id() == PRODUCT_ID_SILHOUETTE_CAMEO3:
            if not pen:
                ...
                s.write("FC0,1,%d\x03FC%d,1,%d\x03" %
                        (toolholder, circle, toolholder))
    ...
    def plot(s, mediawidth=210.0, mediaheight=297.0, margintop=None,
        ...
        """
        bbox = {}
        if margintop is None and 'margin_top_mm' in s.hardware:
            margintop = s.hardware['margin_top_mm']
        if marginleft is None and 'margin_left_mm' in s.hardware:
            marginleft = s.hardware['margin_left_mm']
        ...
        print("mediabox: (%g,%g)-(%g,%g)" %
              (marginleft, margintop, mediawidth, mediaheight), file=s.log)
        ...
        if regmark:
            ...
            #regoriginx = bb['llx']
            #regoriginy = bb['ury']
            print("bb regoriginx=%g regoriginy=%g" %
                  (regoriginx, regoriginy), file=s.log)
            ...
            # only with registration (it was TB50,1), landscape mode
            s.write("TB50,0\x03")
            s.write("TB99\x03")
            # type of regmarks: 0='Original,SD', 2='Cameo,Portrait'
            s.write("TB52,2\x03")
            ...
            if regsearch:
                ...
                s.write("TB123,%i,%i,%i,%i\x03" % (reglength * 20.0, regwidth *
                                                   20.0, (regoriginy - 10) * 20.0, (regoriginx - 10) * 20.0))
            else:
            ...
            if resp != "    0\x03":
                raise ValueError(
                    "Couldn't find registration marks. %s" % str(resp))
        ...
        if s.product_id() == PRODUCT_ID_SILHOUETTE_CAMEO3:
            ...
            p = "\\0,0\x03Z%d,%d\x03L0\x03FE0,0\x03FF0,0,0\x03" % (
                height, width)
            s.write(p)
        ...
        if not 'llx' in bbox:
            bbox['llx'] = 0  # survive empty pathlist
        if not 'lly' in bbox:
            bbox['lly'] = 0
        if not 'urx' in bbox:
            bbox['urx'] = 0
        ...
        if endposition == 'start':
            ...
            # ! axis swapped when using Cameo-system
            new_home = "M%d,%d\x03SO0\x03" % (
                int(0.5+bbox['lly']+end_paper_offset*20.), 0)
    ...
    def load_dumpfile(s, file):
        ...
        for line in open(file, 'r').readlines():
            if re.match(r'\s*\[', line):
                ...
                print(line)
                print(
                    "Error: xml/svg file. Please load into inkscape. Use extensions -> export -> sendto silhouette, [x] dump to file")
                return None
            else:
                print(line, end='')
        return data1234
