...
def translate_poly(poly, xoff, yoff, scale=1):
    tuplepath = []
    for i in poly:
        tuplepath.append(tuple([i[0]*scale+xoff, i[1]*scale+yoff]))
    return tuplepath
...
def show_poly(canvas, path=[(0, 0), (20, 0), (10, 20), (0, 0)], xoff=0, yoff=0):
    ...
    # cannot handle 2-element lists, need 2-element tuples.
    p = Points(tuplepath)
    poly = Polyline(parent=canvas, points=p,
                    line_width=0.5, stroke_color="black")
    ...
    for C in path:
        ...
        text.translate(C[0]+xoff+random.uniform(-.5, 0),
                       C[1]+yoff+random.uniform(-.5, 0))
        text.scale(.25, .25)
...
def show_char(canvas, face, char, x, y, scale, flags=None):
    if not flags:
        flags = freetype.FT_LOAD_IGNORE_GLOBAL_ADVANCE_WIDTH | freetype.FT_LOAD_RENDER | freetype.FT_LOAD_FORCE_AUTOHINT

    idx = face.get_char_index(char)
    # Do not use load_glyph(), it scales all chars to equal height.
    face.load_char(char)
    # need NO_SCALE,  or its broken.
    adv = face.get_advance(idx, flags | freetype.FT_LOAD_NO_SCALE)

    # bbox = face.glyph.outline.get_bbox()
    polys, xadv, yadv = polygons_from_glyph(face.glyph, x=x, y=y, xscale=scale)
    for poly in polys:
        show_poly(canvas, poly)
    ...
    if False:
        # 0.06 for FreeSans. 0.03 for motorhead. this is horrible.
        fix_adv_scale = scale*0.03
        x += adv * fix_adv_scale
        print("advance "+char+" ", adv*fix_adv_scale)
...
def scale_up(win, ev):
    s = canvas.get_scale()
    if chr(ev.keyval) == '+':
        canvas.set_scale(s*1.2)
    elif chr(ev.keyval) == '-':
        canvas.set_scale(s*.8)
    else:
        gtk.main_quit()
...
for char in "0123456789:":
    face.load_char(char)
    p, xa, ya = polygons_from_glyph(
        face.glyph, 0, 0, xscale=-scale, yscale=scale)
...
# enough to show 20mm of the latest drawing on the far side of the device.
tmp_fwd = 85
...
while True:
    ...
    for poly in clock_path:
        clock_path_origin.append(translate_poly(
            poly, -cbox['llx'], -cbox['ury']+cy_off))
    ...
    while (now < when):
        ...
        if not 'name' in doing and len(todo) and todo_needs_origin_moved == 0:
            ...
            if doing['data'] is None:
                ...
                doing['cmds'] = dev.plot_cmds(
                    doing['data'], meta['bbox'], clock_margin, 0)  # keep space for the clock
