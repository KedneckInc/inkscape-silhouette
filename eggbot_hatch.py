    ...
    def recursivelyTraverseSvg(self, aNodeList,
        ...
        for node in aNodeList:
            ...
            if node.tag == inkex.addNS('g', 'svg') or node.tag == 'g':
                ...
                pa = pl.split()
                d = "".join(["M " + pa[i] if i == 0 else " L " + pa[i]
                ...
                inkex.errormsg(
                    'Warning: unable to draw object <%s>, please convert it to a path first.' % node.tag)
                pass
    ...
    def effect(self):
        ...
        if self.options.ids:
            ...
            for id in self.options.ids:
                self.recursivelyTraverseSvg(
                    [self.selected[id]], self.docTransform)
        else:
        ...
        for h in self.grid:
            interstices((h[0], h[1]), (h[2], h[3]), self.paths,
                        self.hatches, self.options.hatchMargin)
