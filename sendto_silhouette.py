...
class SendtoSilhouette(inkex.Effect):
    ...
    def __init__(self):
        ...
        # True: all except hidden layers. False: only selected layers.
        self.allLayers = True
        ...
        self.arg_parser.add_argument('-C', '--cuttingmat', action='store',
        ...
        self.arg_parser.add_argument('--dummy',
                                     action='store', dest='dummy', type=inkex.utils.Boolean, default=False,
        ...
        self.arg_parser.add_argument('-g', '--strategy',
                                     ...
                                     choices=(
                                         'mintravel', 'mintravelfull', 'matfree', 'zorder'),
                                     help="Cutting Strategy: mintravel, mintravelfull, matfree or zorder")
        self.arg_parser.add_argument('-l', '--sw_clipping',
                                     action='store', dest='sw_clipping', type=inkex.utils.Boolean, default=True,
        ...
        self.arg_parser.add_argument('-P', '--sharpencorners',
                                     action='store', dest='sharpencorners', type=inkex.utils.Boolean, default=False,
        ...
        self.arg_parser.add_argument('-r', '--reversetoggle',
                                     action='store', dest='reversetoggle', type=inkex.utils.Boolean, default=False,
        ...
        self.arg_parser.add_argument('-t', '--tool', action='store',
                                     choices=('autoblade', 'cut', 'pen', 'default'), dest='tool', default=None, help="Optimize for pen or knive")
        self.arg_parser.add_argument('-T', '--toolholder', action='store',
        ...
        self.arg_parser.add_argument('-w', '--wait', '--wait-done', '--wait_done',
                                     action='store', dest='wait_done', type=inkex.utils.Boolean, default=False,
                                     help='After sending wait til device reports ready')
        self.arg_parser.add_argument('-x', '--x-off', '--x_off', action='store',
                                     type=float, dest='x_off', default=0.0, help="X-Offset [mm]")
        self.arg_parser.add_argument('-y', '--y-off', '--y_off', action='store',
                                     type=float, dest='y_off', default=0.0, help="Y-Offset [mm]")
        self.arg_parser.add_argument('-R', '--regmark',
                                     action='store', dest='regmark', type=inkex.utils.Boolean, default=False,
                                     help="The document has registration marks.")
        self.arg_parser.add_argument('--regsearch',
                                     action='store', dest='regsearch', type=inkex.utils.Boolean, default=False,
                                     help="Search for the registration marks.")
        self.arg_parser.add_argument('-X', '--reg-x', '--regwidth', action='store',
                                     type=float, dest='regwidth', default=180.0, help="X mark distance [mm]")
        self.arg_parser.add_argument('-Y', '--reg-y', '--reglength', action='store',
                                     type=float, dest='reglength', default=230.0, help="Y mark distance [mm]")
        self.arg_parser.add_argument('--rego-x',  '--regoriginx', action='store',
                                     type=float, dest='regoriginx', default=15.0, help="X mark origin from left [mm]")
        self.arg_parser.add_argument('--rego-y', '--regoriginy', action='store',
        ...
        for node in aNodeList:
            ...
            if node.tag == inkex.addNS('g', 'svg') or node.tag == 'g':
                    ...
                    if refnode:
                        ...
                        if (x != 0) or (y != 0):
                            transform = composeTransform(
                                transform, parseTransform('translate(%f,%f)' % (x, y)))
                        v = node.get('visibility', v)
                        self.recursivelyTraverseSvg(
                            refnode, parent_visibility=v, extra_transform=transform)
                    else:
                        pass
                else:
                    pass
                ...
                if self.resumeMode and (self.pathcount < self.svgLastPath):
                    ...
                    # an "index" for resuming plots quickly-- record last complete path
    ...
    def effect(self):
        ...
        def write_progress(done, total, msg):
            ...
            self.tty.write("%d%%%s %s\r" %
                           (perc-self.device_buffer_perc+.5, buf, msg))
            self.tty.flush()

        try:
            dev = SilhouetteCameo(
                log=self.tty, progress_cb=write_progress, no_device=self.options.dummy)
        ...
        if self.options.strategy == 'matfree':
            ...
            # inkscape crashes whenever something appears in stdout.
            mf.verbose = 0
            ...
            if (overcut > 0) and self.is_closed_path(mm_path):
                ...
                for pprev in reversed(mm_path[:-1]):
                    ...
                    if (precut > dist):  # Full segment needed
                        ...
                        pprev = (pfrom[0]+dx*(precut/dist),
                                 pfrom[1]+dy*(precut/dist))
                        multipath.insert(0, pprev)
                        break
                pfrom = mm_path[0]
                for pnext in mm_path[1:]:
                    ...
                    if (overcut > dist):  # Full segment needed
                        ...
                        pnext = (pfrom[0]+dx*(overcut/dist),
                                 pfrom[1]+dy*(overcut/dist))
                        multipath.append(pnext)
                        break
        ...
        if self.options.autocrop:
            ...
            bbox = dev.plot(pathlist=cut,
                            ...
                            # only return the bbox, do not draw it.
                            bboxonly=None,
                            endposition='start',
        ...
        if len(bbox['bbox'].keys()) == 0:
            ...
            if write_duration > 1.0:
                percent_per_sec = (
                    100.0-self.device_buffer_perc) / write_duration
            else:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        tmpfile = tempfile.NamedTemporaryFile(
            suffix='.svg', prefix='inkscape-silhouette')
