...
extensions_dir = os.path.join(os.path.expanduser(
    "~"), ".config", "inkscape", "extensions")
extension_files = ["sendto_silhouette.inx",
                   "sendto_silhouette.py", "silhouette"]
...
def install_inkscape_silhouette():
    try:
        ...
        logger.warning(
            "inkscape_silhouette install was unsuccessful. Please check previous messages for the cause. Details: %s", ex)
    ...
    for prerequisite in prerequisites:
        ...
        try:
            return_code = subprocess.call(
                "easy_install {}".format(prerequisite), shell=True)
            if return_code > 0:
                raise OSError(
                    "command returned code {}, try running again using sudo".format(return_code))
        except OSError:
            logger.error(
                "unable to install module. Try running 'easy_install %s' manually", prerequisite)
            raise
...
def check_libusb():
    ...
    try:
        ...
        logger.error(
            "pyusb is not installed. Install script usually fails here on first attempt. Running it again should allow installation to complete.")
        raise
    ...
    try:
        ...
        logger.error(
            "libusb is probably not installed. Refer to the instructions in README.md to install it.")
        raise
    except Exception as ex:
        logger.error(
            "something is not right with either pyusb or libusb. Details: %s", ex)
        raise
    ...
    for file in extension_files:
        ...
        if os.path.isfile(path):
            ...
            if os.path.isdir(destination_dir):
                logger.info(
                    "directory already exists '%s'. Removing it and recreating it.", destination_dir)
...
def uninstall_extension():
    ...
    for file in extension_files:
        ...
        try:
            ...
            logger.info(
                "unable to delete %s. It may have been previously removed or was never installed", file_path)
